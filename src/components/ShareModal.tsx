...
const ShareModal = ({ isOpen, onClose, productName, productUrl }: ShareModalProps) => {
  ...
  const shareOptions = [
    {
      ...
      url: `https://wa.me/?text=${encodeURIComponent(`Check out ${productName}: ${productUrl}`)}`,
      color: "bg-[#25D366]",
    },
    {
      name: "Facebook",
    ...
    },
    {
      ...
      url: null, // Instagram doesn't support direct sharing
      color: "bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#F77737]",
    },
    {
      name: "Email",
      icon: <Mail className="w-6 h-6" />,
      url: `mailto:?subject=${encodeURIComponent(`Check out ${productName}`)}&body=${encodeURIComponent(`I thought you might like this: ${productUrl}`)}`,
      color: "bg-gray-600",
    },
  ];
              ...
              <div className="grid grid-cols-5 gap-3 mb-6">
                {shareOptions.map((option) => (
                  <button
                    ...
                    className={`${option.color} p-3 rounded-full text-white hover:opacity-90 transition-opacity flex items-center justify-center`}
                    title={option.name}
                  >
                    {option.icon}
                  </button>
